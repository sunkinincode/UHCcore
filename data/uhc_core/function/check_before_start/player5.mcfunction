...
execute if score Players UHCcore.PlayerAmount matches 5.. run execute as @s[scores={UHCcore.startTriggered=0}] run tellraw @s [{"text":"If you sure to start a UHC.You can tyoe /trigger UHCstart again to start.","color":"green"}]
...
execute unless score Players UHCcore.PlayerAmount matches 5.. run tellraw @s [{"text":"Amount of player who want to play has not enough","color":"gold"}]
